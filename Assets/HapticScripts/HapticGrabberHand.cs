...
public class HapticGrabberHand : MonoBehaviour
{
    ...
    public UnityEvent<string> onPegGrab;
    public UnityEvent onPegRelease;
    ...
    private void ModelSwitch(int modelN)
    {
        ...
        GameObject rOpenHand = grabberParent.transform.GetChild(0).gameObject;
        GameObject rClosedHand = grabberParent.transform.GetChild(1).gameObject;
        GameObject lOpenHand = grabberParent.transform.GetChild(2).gameObject;
        GameObject lClosedHand = grabberParent.transform.GetChild(3).gameObject;


        if (openHand == 1 && rightHand) // Mano derecha abierta
        {
            rOpenHand.SetActive(true);
            rClosedHand.SetActive(false);
            lOpenHand.SetActive(false);
            lClosedHand.SetActive(false);
        }
        else if (openHand == 2 && rightHand) // Mano derecha cerrada
        {
            rOpenHand.SetActive(false);
            rClosedHand.SetActive(true);
            lOpenHand.SetActive(false);
            lClosedHand.SetActive(false);
        }
    ...
    void grab()
    {
        ...
        if (grabbing != null)
        {
            GameObject peg;
            if (grabbing.transform.parent == null)
                peg = grabbing.transform.parent.gameObject;
            else
                peg = grabbing;
            onPegGrab.Invoke(peg.name);
        }
