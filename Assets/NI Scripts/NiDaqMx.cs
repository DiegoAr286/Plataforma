...
namespace Janelia
{
    public static class NiDaqMx
    {
        ...
        private static Dictionary<DigitalOutputParams, ulong> _digitalOutputParamsToTaskHandle = new Dictionary<DigitalOutputParams, ulong>();
        private static Dictionary<DigitalInputParams, ulong> _digitalInputParamsToTaskHandle = new Dictionary<DigitalInputParams, ulong>();
        ...
        public static bool CreateDigitalOutput(DigitalOutputParams p, bool allLines = true, int line = 0)
        {
            ...
            _digitalOutputParamsToTaskHandle.Add(p, taskHandle);
        ...
        public static bool WriteDigitalValue(DigitalOutputParams p, uint[] data, ref int numWritten)
        {
            if (!_digitalOutputParamsToTaskHandle.ContainsKey(p))
            {
                _latestError = "Cannot write before outputs are created";
                return false;
            }
        ...
        {
            {
                ...
                _digitalInputParamsToTaskHandle.Add(p, taskHandle);

                // A list of channels is one string, with list elements separated by ', '.
                // https://zone.ni.com/reference/en-XX/help/370466AH-01/mxcncpts/physchannames/
                //string fullChannelNames = String.Join(", ", p.channelNames.Select(c => p.deviceName + "/" + c));

                byte[] physicalChannels = MakeCString("Dev1/port1/line0:3");
                byte[] namesToAssignToChannels = MakeCString("");
                byte[] emptyCString = MakeCString("");
                int status = DAQmxCreateDIChan(taskHandle, physicalChannels, namesToAssignToChannels, DAQmx_Val_ChanPerLine);
                if (!StatusIndicatesSuccess(status))
                {
                    return false;
                }
        ...
        public static bool ReadFromDigitalInput(DigitalInputParams p, ref uint[] buffer, ref int numReadPerChannel, ref int numBytesPerSamp)
        {
            if (!_digitalInputParamsToTaskHandle.ContainsKey(p))
            {
                _latestError = "Cannot read before inputs are created";
                return false;
            }
        ...
        //public static int IndexInDigitalReadBuffer(int channelIndex, int numReadPerChannel, int index = 0)
        //{
        //    return channelIndex * numReadPerChannel + index;
        //}
        ...
        [DllImport("nicaiu")]
        private static extern int DAQmxCreateDOChan(ulong taskHandle, byte[] physicalChannel, byte[] nameToAssignToChannel, uint lineGrouping);

        [DllImport("nicaiu")]
        private static extern int DAQmxCreateDIChan(ulong taskHandle, byte[] physicalChannel, byte[] nameToAssignToChannel, uint lineGrouping);

        [DllImport("nicaiu")]
        private static extern int DAQmxWriteDigitalLines(ulong taskHandle, int numSampsPerChan, bool autoStart, double timeout, uint dataLayout, uint[] writeArray, ref int sampsPerChanWritten, IntPtr reserved);

        [DllImport("nicaiu")]
        private static extern int DAQmxCfgChangeDetectionTiming(ulong taskHandle, byte[] risingEdgeChan, byte[] fallingEdgeChan, uint sampleMode, uint sampsPerChan);

        [DllImport("nicaiu")]
        private static extern int DAQmxReadDigitalLines(ulong taskHandle, int numSampsPerChan, double timeout, uint fillMode, uint[] readArray, uint arraySizeInBytes, ref int sampsPerChanRead, ref int numBytesPerSamp, IntPtr reserved);
