...
public class CueManager : MonoBehaviour
{
    ...
    void Start()
    {
        // Desactiva colisi�n entre los pegs y el plano invisible
        ...
        for (int i = 0; i < HoleBases.Length; i++)
        {
            ...
            PHCollisionEvents[i].onTriggerExit.AddListener(PegExit); // Establece una conexi�n con los eventos de salida de los pegs
        }
        ...
        if (isAnalogAcquisition)
        {
            ...
            bool n = NiDaqMx.CreateDigitalOutput(digitalOutputParams); // Inicializaci�n de variables de conexi�n con placa de adquisici�n
            if (n)
            {
    ...
    void Update()
    {
        if (Input.GetKeyDown(KeyCode.Escape))
        {
            ...
            SceneManager.LoadScene(SceneManager.GetActiveScene().buildIndex - 1); // Salir del test al men� inicial
        ...
        // Al presionar espacio regresa los pegs a su posici�n inicial
    ...
    void PegEnter(Collider col) // M�todo llamado al producirse un evento de entrada de peg
    {
    ...
    void PegExit(Collider col) // M�todo llamado al producirse un evento de salida de peg (no usado)
    {
